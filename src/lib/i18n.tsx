...
const dict = {
  ...
  "dash.chart1": { en: "Yield Over Time — Gross vs Net (X: Days, Y: USD)", "zh-TW": "收益隨時間變化 — 帳面 vs 實得（X：天數 / Y：美元）", "zh-CN": "收益随时间变化 — 账面 vs 实得（X：天数 / Y：美元）" },
  "dash.chart2": { en: "Break-Even Crossover (X: Days, Y: USD)", "zh-TW": "損益兩平交叉點（X：天數 / Y：美元）", "zh-CN": "损益平衡交叉点（X：天数 / Y：美元）" },
  "dash.chart3": { en: "Break-even by Deposit Size (X: Amount, Y: Days)", "zh-TW": "不同存入金額的損益兩平所需天數（X：金額 / Y：天數）", "zh-CN": "不同存入金额的损益平衡所需天数（X：金额 / Y：天数）" },
  "dash.chart3.note": {
  ...
  "dash.chart4": { en: "Effective Net APY by Deposit Size (X: Amount, Y: %)", "zh-TW": "不同金額的有效淨 APY（X：金額 / Y：%）", "zh-CN": "不同金额的有效净 APY（X：金额 / Y：%）" },
  "dash.chart4.note": {
